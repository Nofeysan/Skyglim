#> modify:restore/list/nums/1115106
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 98
...
scoreboard players set base Temp 68
function modify:restore/list/status/def

scoreboard players set base Temp 6
...
data modify storage modify: restore.name set value 'stage.15.l-chest.2.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "leather_chestplate"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "diamond", pattern: "silence"}

# dyed color
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components merge value {"minecraft:dyed_color": 13815762}
