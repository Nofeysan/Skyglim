#> modify:restore/list/nums/1107005
# 
# 1. 基礎値を設定する
...
data modify storage modify: restore.name set value 'stage.7.e-head.1.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "iron_helmet"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "redstone", pattern: "spire"}
