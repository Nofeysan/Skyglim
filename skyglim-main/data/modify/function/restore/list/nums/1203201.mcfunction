#> modify:restore/list/nums/1203201
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 51
...
scoreboard players set base Temp 136
...
data modify storage modify: restore.name set value 'stage.ex3.m-s.sword.3.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/s.sword {id: "feather"}

# custom_model_data
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:custom_model_data".strings set value ["s.s.diamond"]
