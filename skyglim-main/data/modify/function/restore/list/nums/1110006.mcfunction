#> modify:restore/list/nums/1110006
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 37
...
data modify storage modify: restore.name set value 'stage.10.r-chest.1.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "leather_chestplate"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "gold", pattern: "wayfinder"}

# dyed color
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components merge value {"minecraft:dyed_color": 6499249}
