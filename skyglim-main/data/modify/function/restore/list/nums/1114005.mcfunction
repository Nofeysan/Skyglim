#> modify:restore/list/nums/1114005
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 23
function modify:restore/list/status/cd

scoreboard players set base Temp 24
...
data modify storage modify: restore.name set value 'stage.14.e-head.1.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "leather_helmet"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "emerald", pattern: "ward"}

# dyed color
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components merge value {"minecraft:dyed_color": 5368190}
