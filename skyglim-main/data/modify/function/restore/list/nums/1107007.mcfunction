#> modify:restore/list/nums/1107007
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 39
...
data modify storage modify: restore.name set value 'stage.7.e-legs.1.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "iron_leggings"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "gold", pattern: "spire"}
