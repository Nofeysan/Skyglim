#> modify:restore/list/nums/1114105
# 
# 1. 基礎値を設定する
...
data modify storage modify: restore.name set value 'stage.14.e-head.2.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "leather_helmet"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "copper", pattern: "wayfinder"}

# dyed color
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components merge value {"minecraft:dyed_color": 14818846}
