#> modify:restore/list/nums/1202105
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 52
...
scoreboard players set base Temp 29
...
data modify storage modify: restore.name set value 'stage.ex2.e-head.2.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "leather_helmet"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "copper", pattern: "raiser"}
