#> modify:restore/list/nums/1202106
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 88
...
scoreboard players set base Temp 28
function modify:restore/list/status/cd

scoreboard players set base Temp 60
...
data modify storage modify: restore.name set value 'stage.ex2.e-chest.2.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "leather_chestplate"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "gold", pattern: "raiser"}

# dyed color
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components merge value {"minecraft:dyed_color": 4919569}
