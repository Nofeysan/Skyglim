#> modify:restore/list/nums/1115205
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 14
...
data modify storage modify: restore.name set value 'stage.15.l-head.3.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/armor {id: "netherite_helmet"}

# trim
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:trim" merge value {material: "gold", pattern: "tide"}
