#> modify:restore/list/nums/1203110
# 
# 1. 基礎値を設定する
...
scoreboard players set base Temp 320
...
data modify storage modify: restore.name set value 'stage.ex3.m-shard.21.name'

##* 武器種ごとの共通設定
function modify:restore/list/nums/type/shard {id: "clock"}

# custom_model_data
data modify block ~ ~-1 ~ Items[{Slot: 13b}].components."minecraft:custom_model_data".strings set value ["1203110"]
