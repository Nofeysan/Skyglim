...
execute store result storage modify: setlore.roll int 1 run data get block ~ ~-1 ~ Items[{Slot:13b}].components."minecraft:custom_data".data.roll.damage

###* 実際に lore を増やす
# roll がある = appraise 2 の時
execute if score appraise Temp matches 2 run function modify:refinement/list/status/roll/dmg with storage modify: setlore

# roll がない時
execute unless score appraise Temp matches 2 run function modify:refinement/list/status/no_roll/dmg with storage modify: setlore
