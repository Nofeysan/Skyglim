...
# macro で lore 付与
execute store result storage modify: appraise.rate int 1 run scoreboard players get rate Temp
execute store result storage modify: appraise.value int 0.01 run scoreboard players get base Temp

function modify:appraise/list/macro/status/dmg with storage modify: appraise
...
# roll に乱数を保存する
execute store result block ~ ~-1 ~ Items[{Slot:13b}].components."minecraft:custom_data".data.roll.damage int 1 run scoreboard players get rate Temp
